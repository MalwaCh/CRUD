...
<body>
    <div class="bg-dark py-3">
        <h3 class="text-white text-center">Kajian kamis malam</h3>
    </div>
                    ...
                    <form action="{{route('user.update',$user->id)}}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="card-body">
                            <div class="mb-3">
                                ...
                                <input type="text" value="{{old('name',$user->name)}}" class="form-control form-control-lg  @error('name') is-invalid @enderror" placeholder="Name" name="name" />
                                @error('name')
                            ...
                            <div class="mb-3">
                                ...
                                <input type="email" value="{{old('email',$user->email)}}" class="form-control form-control-lg @error('email') is-invalid @enderror" placeholder="Email" name="email" />
                                @error('email')
                                <p class='invalid-feedback'>{{$message}}</p>
                                @enderror
                            </div>
                            <div class="mb-3">
                                ...
                                <input type="number" value="{{old('phone',$user->phone)}}" class="form-control form-control-lg @error('phone') is-invalid @enderror" placeholder="Phone" name="phone" />
                                @error('phone')
                                <p class='invalid-feedback'>{{$message}}</p>
                                @enderror
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control form-control-lg @error('bio') is-invalid @enderror" placeholder="Bio Data" name="bio" cols="30" rows="5">{{old('bio',$user->bio)}}</textarea>
                                @error('bio')
                                <p class='invalid-feedback'>{{$message}}</p>
                                @enderror
                            </div>
                            <div class="mb-5">
                                ...
                                @if($user->profile_pic !="")
                                <img src="{{asset('uploads/profiles/'.$user->profile_pic)}}" class="img-thumbnail rounded w-25 my-3">
                                @endif
